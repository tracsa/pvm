...
def test_xml_validation_undefined_form():
    with pytest.raises(MalformedProcess) as cm:
        _validate_file('xml/invalid/condition_undefined_form.2018-07-10.xml')

    assert str(cm.value) == \
        'xml/invalid/condition_undefined_form.2018-07-10.xml:26 variable ' \
        'used in condition is not defined \'misterio.password\''
